...
const JobEditCompany = () => {
    ...
    const handleSubmit = async(e) => {
        ...
        jobData.jobStartDate = new Date(job?.jobStartDate).getTime();
        jobData.provinceId = job?.provinceId;
        jobData.noOfVacancies = job?.noOfVacancies;
        jobData.positionId = job?.positionId;
        jobData.jobCategoryId = job?.jobCategoryId;
        jobData.jobTypeId = job?.jobTypeId;
        jobData.salary = job?.salary;
        jobData.requirements = job?.requirements.map((i) => {
            return ({
                content: i?.content,
            });
        });
        ...
        if(!!jobData){
            try {
                const response = await axiosPrivate.put(`company/api/job/update`, { ...jobData });
    ...
    const getJob = async() => {
        ...
        try {
            ...
            console.log(response);
            if(response.status == 200) {
                job.id = response?.data.data.id
                job.title = response?.data.data.title;
                job.description = response?.data.data.description;
                job.address = response?.data.data.address;
                job.jobStartDate = response?.data.data.jobStartDate;
                job.provinceId = response?.data.data.province.id;
                job.noOfVacancies = response?.data.data.noOfVacancies;
                job.positionId = response?.data.data.position.id;
                job.jobCategoryId = response?.data.data.jobCategory.id;
                job.jobTypeId = response?.data.data.jobType.id;
                job.salary = response?.data.data.salary;
                job.requirements = response?.data.data.requirements;
            }
        } catch(err) {
            console.log(err);
        } 
        ...
        <>
            ...
            <div className='with-sidebar-container'>
                <div className='mb-4 dashboard-header'>
                    <div className='title'>
                        <h4 className="m-0 py-2 h1">Edit Job</h4>
                        <Breadcrumb className='ml-2'>
                            ...
                            <BreadcrumbItem><Link to={-1}>Job Detail Management</Link></BreadcrumbItem>
                            <BreadcrumbItem active>Edit job</BreadcrumbItem>
                        </Breadcrumb>
                    </div>
                </div>
