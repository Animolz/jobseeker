...
const JobByPositions = () => {
    const axiosPrivate = useAxiosPrivate();
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    const [data, setData] = useState({});
    const [stats, setStats] = useState({});
    const jobByPositionCols = [
        {
    ...
    const getJobByPositions = async() => {
        setLoading(true);

        try {
            const response =  await axiosPrivate.get('admin/api/stats/jobs-by-position');
            ...
            if(response.status == 200){
                ...
                response?.data.data.forEach(element => {
                    labels.push(element?.positionName);
    ...
    return (
        <>
            <AdminSidebar jobByPosition={true} />
                ...
                <div className='admin-users-management__table'>
                    ...
                    {!loading && !error && !!data?.length && (
                        <Row className="box-shadow-1 bg-light rounded p-1">
                            <Col className="">
                                <MaterialTable title={'Jobs By Position'} columns={jobByPositionCols} data={data}
                                    options={{
                                        minBodyHeight: '80vh',
                                        maxBodyHeight: '80vh',
                                        pageSizeOptions: [],
