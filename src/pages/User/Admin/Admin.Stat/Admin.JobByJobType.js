...
const JobByJobTypes = () => {
    ...
    const getJobByJobType = async() => {
        setLoading(true);

        try {
            const response =  await axiosPrivate.get('admin/api/stats/jobs-by-job-type');
            ...
            if(response.status == 200){
                ...
                response?.data.data.forEach(element => {
                    labels.push(element?.jobTypeName);
    ...
    return (
        <>
            <AdminSidebar jobByJobType={true} />
                ...
                <div className='admin-users-management__table'>
                    ...
                    {!loading && !error && !!data?.length && (
                        <Row className="box-shadow-1 bg-light rounded p-1">
                            <Col className="">
                                <MaterialTable title={'Jobs By Job Type'} columns={jobByJobTypeCols} data={data}
                                    options={{
                                        minBodyHeight: '80vh',
                                        maxBodyHeight: '80vh',
                                        pageSizeOptions: [],
